...
def give_generators(config):
    ...
    p.rotate(0.7, max_left_rotation=5, max_right_rotation=5)
    p.flip_top_bottom(0.5)
    p.flip_left_right(probability=0.5)
    p.random_distortion(probability=0.9, grid_width=4, grid_height=4, magnitude=8)    
    train_generator = p.keras_generator_from_array(all_images, labels, batch_size=config['nbr_image_by_batch'])
